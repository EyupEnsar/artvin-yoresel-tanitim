...
export default function AccommodationListPage() {
  ...
  async function load() { const r = await fetch('/api/accommodation'); setItems(await r.json()) }
  useEffect(() => { load() }, [])

  async function del(id: string) {
    if (!confirm('Silmek istediğine emin misin?')) return
    await fetch(`/api/accommodation/${id}`, { method: 'DELETE' }); load()
  }

  async function toggleStatus(item: any) {
    const next = item.status === 'published' ? 'draft' : 'published'
    await fetch(`/api/accommodation/${item._id}`, {
      method: 'PUT',
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold">Konaklama</h1>
        <Link href="/admin/accommodation/new" className="bg-gray-900 text-white px-4 py-2 rounded text-sm hover:bg-gray-700">+ Yeni Ekle</Link>
      </div>
            ...
            {items.map(item => (
              <tr key={item._id} className="border-b hover:bg-gray-50">
                ...
                <td className="px-4 py-3 text-right space-x-2">
                  <Link href={`/admin/accommodation/${item._id}`} className="text-blue-600 hover:underline">Düzenle</Link>
