...
export default function EventsListPage() {
  ...
  async function load() { const r = await fetch('/api/events'); setItems(await r.json()) }
  useEffect(() => { load() }, [])
  ...
  async function toggleStatus(item: any) {
    ...
    await fetch(`/api/events/${item._id}`, {
      method: 'PUT',
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        ...
        <Link href="/admin/events/new" className="bg-gray-900 text-white px-4 py-2 rounded text-sm hover:bg-gray-700">+ Yeni Ekle</Link>
      </div>
          ...
          <tbody>
            {items.map(item => (
              <tr key={item._id} className="border-b hover:bg-gray-50">
                ...
                <td className="px-4 py-3 text-gray-700">{item.startDate ? new Date(item.startDate).toLocaleDateString('tr') : '—'}</td>
                ...
                <td className="px-4 py-3 text-right space-x-2">
                  <Link href={`/admin/events/${item._id}`} className="text-blue-600 hover:underline">Düzenle</Link>
