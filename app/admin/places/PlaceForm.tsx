...
export default function PlaceForm({ paramsPromise }: Props) {
  const params = paramsPromise ? use(paramsPromise) : null
  const id = params?.id
  const router = useRouter()
  const [loading, setLoading] = useState(false)
  const [form, setForm] = useState({
    title: '', titleEn: '', slug: '', category: '', description: '', descriptionEn: '',
    images: [] as string[], location: '', isFeatured: false, status: 'published',
    lat: null as number | null, lng: null as number | null,
  })

  useEffect(() => {
    if (id) fetch(`/api/places/${id}`).then(r => r.json()).then(d => setForm({
      title: d.title||'', titleEn: d.titleEn||'', slug: d.slug||'', category: d.category||'',
      description: d.description||'', descriptionEn: d.descriptionEn||'',
      images: d.images||[], location: d.location||'', isFeatured: !!d.isFeatured, status: d.status || 'published',
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    await fetch(id ? `/api/places/${id}` : '/api/places', {
      method: id ? 'PUT' : 'POST',
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold mb-6">{id ? 'Yer Düzenle' : 'Yeni Yer'}</h1>
      <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow p-6 space-y-4 max-w-2xl">
        ...
        <div><label className="block text-sm font-medium mb-1">Kategori</label>
          <select value={form.category} onChange={e => setForm(f => ({ ...f, category: e.target.value }))} className="w-full border rounded px-3 py-2 text-sm text-gray-900 bg-white">
            <option value="">Seç</option>
            {['doğa','tarih','şelale','yayla','köy','diğer'].map(c => <option key={c} value={c}>{c}</option>)}
          </select></div>
        <div><label className="block text-sm font-medium mb-1">Konum (Metin)</label>
          <input value={form.location} onChange={e => setForm(f => ({ ...f, location: e.target.value }))} className="w-full border rounded px-3 py-2 text-sm text-gray-900 bg-white" /></div>
        ...
        <div>
          ...
          <ImageUploader type="places" images={form.images} onChange={imgs => setForm(f => ({ ...f, images: imgs }))} />
        </div>
        <div>
        ...
        <div className="flex items-center gap-6">
          <label className="flex items-center gap-2 text-sm">
            <input type="checkbox" checked={form.isFeatured} onChange={e => setForm(f => ({ ...f, isFeatured: e.target.checked }))} /> Öne Çıkar
          </label>
