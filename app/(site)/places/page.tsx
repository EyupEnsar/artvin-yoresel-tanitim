import Image from 'next/image'
import Link from 'next/link'
import connectDB from '@/lib/mongodb'
import Place from '@/lib/models/Place'

export const dynamic = 'force-dynamic'

export default async function PlacesPage() {
  await connectDB()
  const places = await Place.find({ status: 'published' }).sort({ isFeatured: -1, createdAt: -1 }).lean()

  return <PlacesContent places={places} />
}

function PlacesContent({ places }: { places: any[] }) {
  return (
    <div className="max-w-7xl mx-auto px-4 py-12">
      <h1 className="text-4xl font-bold text-gray-800">Gezilecek Yerler</h1>
      <p className="mt-2 text-gray-600">Doğa harikaları ve tarihi yerler</p>

      <div className="mt-10 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {places.map((place: any) => (
          <Link key={place.slug} href={`/places/${place.slug}`} className="group rounded-2xl overflow-hidden shadow hover:shadow-lg transition-shadow bg-white">
            {place.images?.[0] && (
              <div className="relative h-52">
                <Image src={place.images[0]} alt={place.title} fill className="object-cover group-hover:scale-105 transition-transform duration-300" />
                <span className="absolute top-3 left-3 bg-green-700 text-white text-xs px-2 py-1 rounded-full">
                  {place.category}
                </span>
              </div>
            )}
            <div className="p-4">
              <p className="text-xs text-gray-600 mb-1">{place.location}</p>
              <h2 className="text-lg font-bold text-gray-800">{place.title}</h2>
              {place.description && <p className="mt-1 text-sm text-gray-700 line-clamp-2">{place.description}</p>}
              <span className="mt-3 inline-block text-sm text-green-700 font-medium">Devamını Oku →</span>
            </div>
          </Link>
        ))}
        {places.length === 0 && <p className="col-span-3 text-center text-gray-600 py-12">İçerik bulunamadı.</p>}
      </div>
    </div>
  )
}
